...
struct Face {
	...
	std::vector<cv::Point> leftEye, rightEye, noseRidge, chin, leftEyebrow,
			rightEyebrow;
	Face() {
	...
	void drawOnFrame(Frame frame) {
		...
		for (int i = 0; i < this->landmarks.size(); i++) {
			try {
				cv::circle(frame, this->landmarks[i], radius, YELLOW_COLOR, -1);
			} catch (...) {
			}
			try {
				cv::putText(frame, std::to_string(i), this->landmarks[i],
						cv::FONT_HERSHEY_SIMPLEX, 0.5, CYAN_COLOR);
			} catch (...) {
			}
		}
